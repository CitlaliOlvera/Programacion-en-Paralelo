...
tmax <- 400 #duracion de la simulacion
...
for(i in 1:tmax){
  ...
  for(g in 1: cant.camiones){ #actualiza la posicion del camion
  ...
  for(w in 1:cant.camiones){#revisaremos si en la posicion que se encuentra cada camion es una parada, si es así baja y sube pasaje
   ...
   # if(camiones[w,1]==t.estimado.llegada[1]){
    ...
    #ACCIDENTE
    for(g in 1: cant.camiones){#cualquier camion que este realizando un recorrido puede tener un accidente
      if(camiones[g,cant.paradas+2]==0){#es un camion que todavia esta realizando el recorrido
        if(runif(1) < pa){
          camiones[g,1] <- camiones[g,1]- sample(1:10,1) #se agrega el tiempo de accidente
          if(camiones[g,1] < 0){#que  no haya posiciones negativas 
            camiones[g,1] <- 0
          }
        }
      }
...
tiempoSec <- c(tiempoSec, tiempoEjecucion)
}
T.Espera <- T.Espera[-1,]
write.csv(tiempoSec , file = "tiempoSecuencial400.csv")